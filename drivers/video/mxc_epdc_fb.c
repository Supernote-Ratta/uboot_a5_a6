...
//	do_update(panel_info.epdc_data.wv_modes.mode_init, 0);
	do_update(panel_info.epdc_data.wv_modes.mode_gc16, 0);
	epdc_power_off();
}
...
#define CCM_CSCDR2 0x20c4038 
/*set pix_clk to 127M */
static void set_epdc_clk(void)
{
	unsigned int reg_val;

	reg_val=REG_RD(CCM_CSCDR2, 0);
	debug("epdc: epdc_pix_sel=%ld\n",reg_val);
	reg_val &= ~(7<<12) ;
	//reg_val |= 1<<12 ;
	reg_val=REG_WR(CCM_CSCDR2, 0, reg_val);
	reg_val=REG_RD(CCM_CSCDR2, 0);
	debug("epdc: epdc_pix_sel=%ld\n",reg_val);
}
